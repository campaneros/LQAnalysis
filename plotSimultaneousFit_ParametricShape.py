...
parser.add_option("-t", "--toysfile", dest="toysfile",
                  help="input file with fitted toys")

parser.add_option("-f", "--fitfile", dest="fitfile",
                  help="input file with tree of post-fit parameters.")

parser.add_option("-c", "--catdir", dest="catdir",
                  help="name of directory containing categories dirs")
                  
parser.add_option("-o", "--outputdir", dest="outputdir",
                  help="name of the output directory")

parser.add_option("-b", "--outputfilename", dest="outputfile",
                  help="name of the output file")

parser.add_option("-w", "--weboutputdir", dest="weboutputdir",
                  help="name of the web output directory")

parser.add_option("-s", "--signaname", dest="signaname",
...
workspace = toysfile.Get(workspacename)
print(workspacename)
#else:
...
for icat,cat in enumerate(categoriesList):
    ...
    hData = roohist_data.createHistogram("hData_"+app, x, RooFit.Binning(b))
    ...
    for ibin, bincenter in enumerate(a):
        ...
        bkg_pdfIntegral = BkgFit.createIntegral(ROOT.RooArgSet(x),RooFit.NormSet(ROOT.RooArgSet(x)),RooFit.Range("binrange_"+str(ibin)))
    ...
    for ibin in range(hData.GetNbinsX()):
        ibin  +=1
        bin_width = float(hData_norm.GetBinWidth(ibin))
    ...
    #else:
    legend.AddEntry(hSignal, "signal (pb) r="+str(round(r,5)) +"#pm  "+str(round(err_r,4)), "l")
    legend.Draw()
    ...
    canvas.SaveAs(opt.outputdir+"/canvas_toy_"+str(nToy)+"_"+app+"_"+opt.outputfile+".png")
