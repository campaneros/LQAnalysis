...
def run_bias_study(args):
    ...
    for function_fit in itertools.product(fitting_function):
     fit_fucntion_name_fit= cwd_utils.nested_dict["function_%s"%function_fit]["%s"%category]
     os.system("python goodness_of_fit,py -d /data/mcampana/CMS/CMSSW_8_1_0_LQ/src/Fit_Signal_BDT_data_all/new_umu_datacards_finalcat_"+function_gen+"_450/umuLQumu_M"+str(m)+"_L"+str(l)+"/categories/datacard_"+fit_fucntion_name_gen+"_umuLQumu_M"+str(m)+"_L"+str(l)+"_"+category+".txt -t "+str(toy)+" --expectSignal "+str(exp)+" -l "+str(limit)+" -o /data/mcampana/CMS/CMSSW_8_1_0_LQ/src/Fit_Signal/output_MC/goodness_fit_"+function_fit+"")
