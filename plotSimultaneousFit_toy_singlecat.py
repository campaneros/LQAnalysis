...
print(cat)
Mass[0] = float(cat_parts[index-2].replace("M",""))
L[0] = float(cat_parts[index-1].strip("L").replace("p","."))
...
for number in range(nToy):
    ...
    hSignal = ROOT.TH1D("hSignal_"+cat,"",NvarBins,a)
    hBkg    = ROOT.TH1D("hBkg_"+cat,"",NvarBins,a)
    ...
    ## Create canvas
    canvas = ROOT.TCanvas("canvas_"+cat, "canvas_"+cat, 200, 10, 700, 500 )
    ...
    print(number)
    canvas.SaveAs(opt.outputdir+"/canvas_toy_"+str(nToy)+"_"+cat+"_"+opt.outputFile+"toyesnumber"+str(number)+".png")
